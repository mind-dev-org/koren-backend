...
$router->group(['prefix' => '/api/v1'], function($r) {
    $r->get('/health', fn($req) => \Vine\Core\Response::success([
        'status'  => 'ok',
        'version' => '1.0.0',
        'db'      => 'connected',
        'time'    => date('c'),
    ...
    $r->group(['prefix' => '/farmer', 'middleware' => FarmerMiddleware::class], function($r) {
        ...
        $r->put('/products/{id}',            [FarmerController::class, 'updateProduct']);
        $r->delete('/products/{id}',         [FarmerController::class, 'deleteProduct']);
        $r->get('/orders',                   [FarmerController::class, 'myOrders']);
        $r->patch('/orders/{id}/status',     [FarmerController::class, 'updateOrderStatus']);
        $r->get('/stats',                    [FarmerController::class, 'stats']);
    });

    $r->post('/orders',              [OrderController::class, 'store']);
    $r->get('/orders/{id}/tracking', [OrderController::class, 'publicTracking']);

    $r->group(['prefix' => '/me', 'middleware' => AuthMiddleware::class], function($r) {
        ...
        $r->get('/orders/{id}/tracking',     [CustomerController::class, 'tracking']);
        $r->get('/points',                   [CustomerController::class, 'points']);
        $r->get('/points/history',           [CustomerController::class, 'pointsHistory']);
        $r->post('/points/redeem',           [CustomerController::class, 'redeemPoints']);
        $r->get('/subscriptions',            [CustomerController::class, 'subscriptions']);
        $r->post('/subscriptions',           [CustomerController::class, 'createSubscription']);
        $r->get('/subscriptions/{id}',       [CustomerController::class, 'subscriptionDetail']);
        $r->patch('/subscriptions/{id}',     [CustomerController::class, 'updateSubscription']);
        $r->delete('/subscriptions/{id}',    [CustomerController::class, 'deleteSubscription']);
    });
