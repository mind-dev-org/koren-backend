...
class CustomerController
{
	public function profile(Request $request): Response
	{
		...
		$pointsToNext = $current["next_min"]
			? $current["next_min"] - $totalEarned
			: null;
		...
		return Response::success([
			...
			"address" => [
				...
				"lat" => $user["address_lat"]
					? (float) $user["address_lat"]
					: null,
				"lng" => $user["address_lng"]
					? (float) $user["address_lng"]
					: null,
			],
			"loyalty" => [
			...
			"stats" => [
				...
				"active_subscriptions" =>
					(int) ($user["active_subscriptions"] ?? 0),
			],
	...
	public function orders(Request $request): Response
	{
		...
		if (!empty($orderIds)) {
			...
			foreach ($items as $item) {
				if (
					!isset($previewMap[$item["order_id"]]) ||
					count($previewMap[$item["order_id"]]) < 3
				) {
	...
	public function points(Request $request): Response
	{
		...
		if ($current["next_min"]) {
			...
			$progressPercent =
				$range > 0
					? min(100, (int) round(($progress / $range) * 100))
					: 100;
		}
	...
	public function updateSubscription(Request $request): Response
	{
		...
		$updateData = match ($action) {
			"pause" => [
				"status" => "paused",
				"pause_until" => $request->input("pause_until"),
			],
			"resume" => ["status" => "active", "pause_until" => null],
			"update" => [
				"frequency" =>
					$request->input("schedule")["frequency"] ??
					$sub["frequency"],
			],
			default => [],
		};
	...
	private function tierMeta(): array
	{
		return [
			...
			"seedling" => [
				...
				"next_min" => 1000,
				"perks" => [
					"Priority delivery on Tue/Thu",
					"+5% points for every order",
				],
			],
			"harvest" => [
				"label" => "Harvest",
				...
				"next" => "root",
				"next_min" => 2500,
				"perks" => ["Free delivery 1 time/month", "+10% points"],
			],
			"root" => [
				"label" => "Root",
				"min" => 2500,
				"next" => null,
				"next_min" => null,
				"perks" => ["VIP support", "+15% points", "Free delivery"],
			],
		];
	}
}
